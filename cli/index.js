...
async function loadIdentity() {
  return JSON.parse(await readFile(IDENTITY_FILE, 'utf8'));
}
...
async function main() {
  ...
  if (command === 'init') {
    ...
    print({ ok: true, nodeId: identity.nodeId, home: DEFAULT_HOME });
    return;
  }

  if (command === 'identity') {
    const identity = await loadIdentity();
    print({ nodeId: identity.nodeId, algorithm: identity.algorithm, publicKey: identity.publicKeyPem });
    return;
  }
  ...
  if (command === 'relay') {
    ...
    print(`TRUYN MVP relay listening on ${url}`);
    return;
  }
  ...
  if (command === 'register') {
    const result = await node.register({ name: argValue('--name') });
  ...
  if (command === 'find') {
    const capability = process.argv[3];
    if (!capability) throw new Error('Usage: truyn find <capability> [--relay URL]');
    print(await node.find(capability));
    return;
  }
  ...
  if (command === 'poll') {
    ...
    node.sessionToken = session.sessionToken;
    print(await node.poll());
    return;
  }
