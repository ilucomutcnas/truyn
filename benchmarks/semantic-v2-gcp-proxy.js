...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === 'GET' && req.url === '/health') return sendJson(res, 200, { ok: true, role: 'semantic-v2-gcp-proxy' });

  const allowedVertexCall = req.method === 'POST'
    && req.url?.startsWith('/v1/projects/')
    && (req.url.endsWith(':generateContent') || req.url.endsWith(':predict'));
  if (!allowedVertexCall) return sendJson(res, 404, { ok: false, error: 'not_found' });
...
server.listen(port, host, () => {
  process.stdout.write(`${JSON.stringify({ ok: true, role: 'semantic-v2-gcp-proxy', port })}\n`);
});
