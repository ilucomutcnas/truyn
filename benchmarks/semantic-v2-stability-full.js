...
const rows = await mapLimit(retrievalCases, 4, async (item, queryIndex) => {
  ...
  return {
    ...
    provenancePass:Boolean(selected),
...
const report = {
  benchmark:'TRUYN Semantic Retrieval Gate v2 stability-hardened full 360 proof',
  ...
  methodology:{
    ...
    stabilityRecheck:'repeat Lite only with reversed candidate order; accept the initial cheap agreement only when Lite resolves to the same original passage, otherwise fail closed to verifier',
    provenanceDefinition:'provenance validates integrity/identity of the selected block; semantic correctness is measured separately by accuracy',
