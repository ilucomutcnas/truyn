...
- provider hot paths: persistent relay-origin WebSockets
...
3. Provider dispatch/results moved to persistent relay-origin WebSockets.
...
Token, cost, and total application-byte reduction remain separate objectives. The next benchmark should target semantic/context compaction on materially larger contexts.
